...
param([int]$Days = 14, [int]$Chunk = 6)
$ErrorActionPreference = "Continue"
$env:Path += ";" + (npm prefix -g).Trim()
New-Item -ItemType Directory -Force data/trades | Out-Null

$now = [int][double]::Parse((Get-Date -UFormat %s))
$end = ([int]([math]::Floor($now / 3600)) - 2) * 3600          # skip the last 2h: not finalized
$rng = New-Object System.Random 42                              # fixed seed: same sample every run
$windows = @()
for ($h = $end - $Days * 86400; $h -lt $end; $h += 3600) {
    $picks = 0..11 | Sort-Object { $rng.Next() } | Select-Object -First 2 | Sort-Object
...
Write-Host "done: $((Get-ChildItem data/trades/chunk_*.csv.gz).Count) chunks"
